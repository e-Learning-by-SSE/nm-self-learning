...
import { fetchAvailableModels } from "./llm-config.router";
import { fetchLlmConfig } from "./ai-tutor.router";
		...
		try {
			const profile = await database.aiTutorProfile.upsert({
				...
				update: {
					...
					avatarUrl: input.profile_pictureUrl,
					systemPrompt: input.system_context,
					model: input.model,
					author: input.author
				},
				create: {
