...
function cleanup() {
	...
	// introspected.prisma is generated by db pull and must be removed after ALL migrations are applied
	const introspectFile = join(schemaPath, "introspected.prisma");
	if (existsSync(introspectFile)) {
		console.log(`Introspection schema detected. ${info}Removing introspection${normal}`);
		// Remove introspected file
		unlinkSync(introspectFile);
	}
