...
import { DynCourseGenPathFormModel } from "@self-learning/types";
...
export function CoursePreview() {
	...
	const hasLessonPath = !!lessonPath;
	...
	const { data: lessonsData } = trpc.lesson.findByIds.useQuery(
		{ lessonIds },
...
function CourseHeader({
	...
	return (
		<section className="flex flex-col gap-16">
			...
			{hasLessonPath && (
				<TableOfContents content={content} course={course} isGenerated={hasLessonPath} />
			)}
		</section>
	);
}
...
function CoursePath({
	...
	const { mutateAsync: generateLessonPath } = trpc.dynCourse.createNewLessonPath.useMutation();
	const router = useRouter();
	const [isGenerating, setIsGenerating] = useState(false);
	const [generationId, setGenerationId] = useState<string | null>(null);
	const [isComplete, setIsComplete] = useState(false);

	const generateNewLessonPath = async () => {
		try {
			...
			console.log("generate new lessonPath, result", result);
	...
	if (needsARefresh) {
		return <RefreshGeneratedCourse onClick={generateNewLessonPath} />;
	}
	return (
		<div>
			{isGenerating && (
			...
			<button
				className="btn-primary mt-4 w-full text-white p-3 rounded-lg flex items-center justify-center font-semibold"
				onClick={generateNewLessonPath}
				disabled={isGenerating}
			>
				{hasLessonPath ? "Kurspfad Neugenerieren" : "Kurspfad Generieren"}
			</button>
		</div>
	);
}
