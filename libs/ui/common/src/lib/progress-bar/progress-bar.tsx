 ...
 * @param hideOnFinish If true, the progress bar will be hidden after the progress has finished
 * @returns A React component to visualize the progress
 */
export function ProgressBar({
	progress,
	hideOnComplete: hideOnFinish = true
}: {
	progress: number;
	hideOnComplete?: boolean;
}) {
	if (progress < 100 || !hideOnFinish) {
		return (
