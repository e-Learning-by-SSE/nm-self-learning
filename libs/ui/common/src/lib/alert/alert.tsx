...
export type warningType = { severity: "WARNING" | "ERROR" | "INFORMATION" | "SUCCESS", message: React.ReactNode };
		...
		>
			<div className="flex flex-row gap-2">
				{type.severity === "WARNING" ? <ExclamationCircleIcon className="icon h-5 text-lg"/> :
					type.severity === "ERROR" ? <ShieldExclamationIcon className="icon h-5 text-lg"/> :
						type.severity === "INFORMATION" ? <InformationCircleIcon className="icon h-5 text-lg"/> :
