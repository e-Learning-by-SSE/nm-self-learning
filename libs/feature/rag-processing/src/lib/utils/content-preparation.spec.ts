...
// content-preparation.ts imports it from "@self-learning/rag-processing" (the
// barrel export), so we mock that same path.
...
describe("prepareRagContent", () => {
    ...
    // =========================================================================
        ...
        it("extracts text from a single article", async () => {
            ...
            const content: LessonContent = [
                { type: "article", meta: { estimatedDuration: 5 }, value: { content: "TypeScript is a typed superset of JavaScript." } }
            ];
        ...
        it("extracts text from multiple articles", async () => {
            // Setup
            const content: LessonContent = [
                { type: "article", meta: { estimatedDuration: 5 }, value: { content: "First article." } },
                { type: "article", meta: { estimatedDuration: 3 }, value: { content: "Second article." } }
            ];
    ...
    // =========================================================================
        ...
        it("passes PDF URLs to downloadMultiple and returns the buffers", async () => {
            // Setup
            const fakePdfBuffer = { data: "base64encodedpdf==", url: "https://example.com/slides.pdf" };
            mockDownloadMultiple.mockResolvedValue([fakePdfBuffer]);
            const content: LessonContent = [
                { type: "pdf", meta: { estimatedDuration: 10 }, value: { url: "https://example.com/slides.pdf" } }
            ];
        ...
        it("does not call downloadMultiple when there are no PDF items", async () => {
            // Setup
            const content: LessonContent = [
                { type: "article", meta: { estimatedDuration: 5 }, value: { content: "Some text." } }
            ];
        ...
        it("passes multiple PDF URLs together in a single downloadMultiple call", async () => {
            ...
            const content: LessonContent = [
                { type: "pdf", meta: { estimatedDuration: 5 }, value: { url: "https://example.com/a.pdf" } },
                { type: "pdf", meta: { estimatedDuration: 5 }, value: { url: "https://example.com/b.pdf" } }
            ];
    ...
    // =========================================================================
        ...
        it("returns empty transcriptTexts when video has no subtitle", async () => {
            // Setup
            const content: LessonContent = [
                { type: "video", meta: { duration: 120 }, value: { url: "https://example.com/video.mp4" } }
            ];
    ...
    // =========================================================================
        ...
        it("strips WEBVTT header and timestamp lines, returning only spoken text", async () => {
            ...
            expect(result.transcriptTexts[0]).toContain("Today we will cover the basics of TypeScript.");
        });
            ...
            const content: LessonContent = [
                {
                    ...
                    value: {
                        ...
                        subtitle: { src: makeVtt("First video text."), label: "Deutsch", srcLang: "de" }
                    }
                },
                {
                    type: "video",
                    ...
                    value: {
                        ...
                        subtitle: { src: makeVtt("Second video text."), label: "Deutsch", srcLang: "de" }
                    }
                }
            ];
    ...
    // =========================================================================
        ...
        it("processes all content types together correctly", async () => {
            ...
            const content: LessonContent = [
                { type: "article", meta: { estimatedDuration: 5 }, value: { content: "Article text." } },
                { type: "pdf", meta: { estimatedDuration: 10 }, value: { url: "https://example.com/doc.pdf" } },
                {
                    type: "video",
                    meta: { duration: 5 },
                    value: {
                ...
                { type: "iframe", meta: { estimatedDuration: 5 }, value: { url: "https://example.com/embed" } }
            ];
