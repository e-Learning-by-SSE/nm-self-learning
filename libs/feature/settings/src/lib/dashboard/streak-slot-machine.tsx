...
type StreakStatus = "active" | "broken" | "refire" | "pause";

const bgBase = "p-8 rounded-xl shadow-xl mb-4";

function StreakMachineAnimated({
	streakStatus,
	children
}: {
	...
	if (streakStatus === "broken") {
		return (
			<div className={`${bgBase}  bg-gradient-to-br from-gray-400 via-gray-400 to-gray-600`}>
				{children}
			</div>
		);
	}
	...
	if (streakStatus === "pause") {
		return (
			// <div className="relative">
			// 	<div className={`${bgFlames} z-10`}>
			// 		<AnimatedFlame className="w-full h-full" isBlue />
			// 	</div>
			// 	<div
			// 		className={`${bgBase} bg-gradient-to-br from-purple-600 via-indigo-600 to-indigo-800 z-10`}
			// 	>
			// 		{children}
			// 	</div>
			// </div>
...
export function StreakSlotMachineDialog({
	...
	const renderSlotCounter = (): React.ReactNode => {
		switch (streakStatus) {
			case "refire":
				return (
					<SlotCounter
		...
		>
			...
			<div className="overflow-y-auto h-full">
				...
				<div className="flex flex-col items-center justify-center mb-6 relative">
					...
					{/* Streak Slot Machine */}
					<StreakMachineAnimated streakStatus={streakStatus}>
