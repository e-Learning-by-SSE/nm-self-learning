...
export async function checkForCycles(skillMap: Map<string, FolderItem>, item?: FolderItem) {
	...
	if (!parents) {
		...
		const onClickDetailedCycle = () => {
			dispatchDialog(
				<SimpleDialog
					description={cycleStackTrace}
					name={"Zyklen - Darstellung"}
					onClose={() => freeDialog("simpleDialog")}
				/>,
				"simpleDialog"
			);
		};
			...
			<Alert
				type={{
					...
					message: (
						<div>
							<button
								onClick={onClickDetailedCycle}
								className="text-left hover:cursor-pointer hover:text-red-700"
							>
								<span>
									Warnung: In Ihrer Modelierung der Lernziele liegt ein Zyclus
									vor. Aufgrunddessen kann der Algorithmus keinen Gültigen
									Lernfpad mit denen im Zyclus beteiligten Lernzielen finden.
									Angegebene Pfadziel sind nicht erreichbar und nur Privat
									speicherbar. Die betroffenden Skills können durch Klicken auf
									diesen Text angezeigt werden.{" "}
								</span>
							</button>
						</div>
					)
