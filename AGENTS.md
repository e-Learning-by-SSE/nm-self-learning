...
### Use Trans component rather than splitting messages into multiple partial strings

When translating a text which is devided into multiple parts, e.g. because some parts are formated differently, use <Trans> components to avoid creation of multiple sub strings

Prefer `Trans` component of `@self-learning/ui/common` over `next-i18next`as this provides predefined tags for commonly used formatings.
