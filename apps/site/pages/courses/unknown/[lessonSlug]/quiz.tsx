...
export const getServerSideProps: GetServerSideProps<QuestionProps> = async ({ params }) => {
	const parentProps = await getStaticPropsForLayoutUnknownCourse(params);
...
export default function QuestionsPage({ course, lesson, quiz, markdown }: QuestionProps) {
	...
	const goToNextQuestion = useCallback(() => {
		router.push(
			`/courses/${unknownCourse.slug}/${lesson.slug}/quiz?index=${nextIndex}`,
			undefined,
			{
				shallow: true
			}
	...
	function goToQuestion(index: number) {
		router.push(
			`/courses/${unknownCourse.slug}/${lesson.slug}/quiz?index=${index}`,
			undefined,
			{
				shallow: true
			}
	...
	return (
		<div className="flex flex-col gap-4">
			<div className="relative flex flex-col gap-2">
				...
				<Link href={`/courses/${unknownCourse.slug}/${lesson.slug}`}>
